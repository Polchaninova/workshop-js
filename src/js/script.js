...
const cardsMarkup = createGalleryItemsMarkup(productCards);

galleryItemsContainer.insertAdjacentHTML("beforeend", cardsMarkup);
function createGalleryItemsMarkup(galleryItems) {
    return galleryItems.map(({ urlToImage, title, tags, content, date }) => {
        return `
            <li class="modal-content gallery__item">
                 <a class="gallery__link" href="${urlToImage}">
                  <img id="modalCardImg" class="strategy__image" src="${urlToImage}" alt="strategy">
                    <p id="modalCardData" class="modal-strategy-data">${date}</p>
                    <h2 id="modalCardName" class="modal-strategy-name">${title}</h2>
                    <p id="modalCardDescription" class="modal-strategy-description">${content}</p>
...
const gallery__item = new SimpleLightbox('.gallery__item a');
